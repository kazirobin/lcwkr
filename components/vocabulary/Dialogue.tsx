        ...
        {dialogue.lines.map((line, index) => (
          ...
          >
            <div className="flex-shrink-0 min-w-[80px]">
              <span
                className={`inline-block px-3 py-1 rounded-full text-xs font-semibold text-white ${getSpeakerColor(line.speaker)}`}
              >
                {line.speaker}
              </span>
            </div>
            ...
            <div className="flex-1 space-y-1">
              <p className="text-lg font-medium text-gray-800">{line.hanzi}</p>
              <p className="text-sm text-gray-600">{line.pinyin}</p>
              <p className="text-sm text-gray-700">{line.english}</p>
            </div>
          </div>
        ))}
      </div>
